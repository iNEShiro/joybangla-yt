...
const filters = {
    bassboost: 'bass=g=20, bass=f=80, dynaudnorm=f=1500',
...
class Player extends EventEmitter {
    ...
    pause (message) {
        ...
        queue.paused = true
		this.emit('queuePaused', message, queue.tracks[0])
    }
    ...
    resume (message) {
        ...
        queue.paused = false
		this.emit('queueResumed', message, queue.tracks[0])
    }
    ...
    setVolume (message, percent) {
        ...
        queue.voiceConnection.dispatcher.setVolumeLogarithmic(queue.calculatedVolume / 100)
    }
    ...
    _handleVoiceStateUpdate (oldState, newState) {
		...
		//joins channel & paused
		if (queue.paused && oldState.channel !== oldState.guild.me.voice.channel && newState.channel === oldState.guild.me.voice.channel && queue.voiceConnection.channel.members.filter((member) => !member.user.bot).size === 1) {
			queue.voiceConnection.dispatcher.resume()
			queue.paused = false
			this.emit('queueResumed', oldState, queue.tracks[0])
		}
		...
		if(queue.voiceConnection.dispatcher) { queue.voiceConnection.dispatcher.pause() }
        queue.paused = true
		this.emit('queuePaused', oldState, queue.tracks[0])
        ...
        }, /*this.options.leaveOnEmptyCooldown ||*/ 180000)
    }
